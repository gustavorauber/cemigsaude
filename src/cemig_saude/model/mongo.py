...
def get_favorites(filter_by={}):
    db = __get_db()
    user_favorites = db['favorites'].find_one(filter_by)
...
def add_favorite(user_id, physician_hash):
    db = __get_db()
    col = db['favorites']

    col.update({'_id': user_id},
                {'$addToSet': {'favorites': physician_hash}}, upsert=True)

def remove_favorite(user_id, physician_hash):
    db = __get_db()
    col = db['favorites']
